...
ephemeral_start() {
    ...
    [ ! -e "/dev/$VG_NAME/tmp" ] && /sbin/lvcreate -L 5G -ntmp "$VG_NAME"
    [ ! -e "/dev/$VG_NAME/cache" ] && /sbin/lvcreate -L 5G -ncache "$VG_NAME"
    ...
    # do data: /var/lib/elasticsearch
    /sbin/mkfs.xfs /dev/$VG_NAME/data
...
ephemeral_stop() {
    ...
    /bin/umount /var/lib/elasticsearch
